    ...
    if ( isset($_GET['categories']) )
    {
        switch ( $_GET['categories'] )
        {
            case 'save':
                ...
                {
                    if ( check_nonce(($_POST['nonce'] ?: ''), SBLOG.'savecategory') == false )
                    {
                        die( 'CSRF detected!' );
                    }
                }
                if ( $SimpleBlog->saveCategory($_POST) )
                {
                    $undolink = '<a href="load.php?id=' . SBLOG . '&categories=restore">' . i18n_r('UNDO') . '</a>';
                    SimpleBlog_displayMessage( i18n_r(SBLOG . '/UI_CATEGORY_SAVED_MSG') . " - " . $undolink, 'success', true );
                }
                else
                {
                    SimpleBlog_displayMessage( i18n_r(SBLOG . '/UI_CATEGORY_NOT_SAVED_MSG'), 'error', false );
                }
                break;

            case 'update':
                // Update changes to a category
                if ( defined('GSNOCSRF') == false || GSNOCSRF == false )
                {
                    if ( check_nonce(($_POST['nonce'] ?: ''), SBLOG.'updatecategory') == false )
                    {
                        die( 'CSRF detected!' );
                    }
                }
            ...
            case 'restore':
                // Restore last change to categories
                break;

            default:
                // Nothing to do
        }
        // Show the category listing
