...
class SimpleBlog
{
    ...
    {
        // Get Category XML data from file
        $category_data = getXML($this->data_files['categories']);
        $category_array = array();

        if ( $category_data ) // @TODO: Determine how to check of we have data here
        {
            foreach ( $category_data as $category_data )
            {
                $category_array[] = array(
                    'slug' => (string) $category_data->slug,
                    'title' => (string) $category_data->title,
                    'description' => (string) $category_data->description,
                    'post-count' => 0
                );
                // @TODO: Get and add post count to array - This should be pulled from the cache
            }
        }
    ...
    public function getCategory( string $slug ): array
    {
        ...
        foreach ( $categories as $category )
        {
            ...
            {
                // This is the requested category
                $category_data = $category;

                $category_data['posts'] = array(
                    // @TODO: Append a list of post slugs attached to this category - This should be pulled from the cache
                );
            }
        }
     ...
     * Saves a category and its data to the categories.xml file. If the category already exists, a new categoy with an
     * incrementing number appended will be created instead. If 'oldslug' is found in the array, the category is being
     * updated; if oldname and newname are the same, update details; if oldname and newname are different, remove
     * category and create a new one.
     *
     * @since 1.0
