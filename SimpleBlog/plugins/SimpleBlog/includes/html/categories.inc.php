    ...
    <table class="highlight">
        <tbody class="list">
            <?php foreach ( $categories as $category ) { ?>
                <tr id="<?php echo $category['slug'] ?: ''; ?>">
                    <td class="name">
                        <span><?php echo $category['title'] ?: 'Untitled Category'; ?></span>
                        <a href="load.php?id=<?php echo SBLOG; ?>&manage&filter=category&keyphrase=<?php echo $category['slug'] ?: ''; ?>" class="posts"><?php echo (string) $category['post-count'] ?: '0'; ?> posts</a>
                        <p><?php echo $category['description'] ?: '<em>No Description Provided</em>'; ?></p>
                    </td>
                    <td class="secondarylink edit">
                        <a href="#" title="Edit Category: <?php echo $category['title'] ?: 'Untitled Category'; ?>" class="gs_simpleblog_ui_edit-button" data-edit="<?php echo $category['slug'] ?: ''; ?>">
                    ...
                    <td class="delete">
                        <a class="delconfirm" href="load.php?id=<?php echo SBLOG; ?>&categories=delete&catid=<?php echo $category['slug'] ?: ''; ?>" title="Delete category: <?php echo $category['title'] ?: 'Untitled Category'; ?>">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" fill="currentColor">
                                <!--! Font Awesome Pro 6.1.1 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2022 Fonticons, Inc. -->
        ...
        <ul class="pagination"<?php if ( count($categories) < $SimpleBlog->getSetting('postsperpage') ) { echo ' style="display:none;"'; } ?>>
...
<script>
    window.addEventListener("DOMContentLoaded", function() {
        ...
        var categoryList = new List('gs_simpleblog_ui_categories_table', {
            ...
            page: <?php echo $SimpleBlog->getSetting('postsperpage'); ?>,
            pagination: true
        });

        // Table sorting buttons
        var sortbuttons = document.getElementsByClassName('gs_simpleblog_ui_sort-button');
        ...
        var editbuttons = document.getElementsByClassName('gs_simpleblog_ui_edit-button');
        for ( i = 0; i < editbuttons.length; i++ ) {
            if ( editbuttons[i].hasAttribute('data-edit') ) {
                editbuttons[i].addEventListener("click", function(e) {
                    let tablerow = document.getElementById(this.getAttribute('data-edit'));
                    let container = tablerow.getElementsByClassName('name')[0];
                    let title = container.getElementsByTagName('span')[0].innerHTML;
                    let description = container.getElementsByTagName('p')[0].innerHTML;

                    // The edit form
                    let editform =  '<form class="largeform">' +
                                    '<input type="hidden" name="oldslug" value="'+this.getAttribute('data-edit')+'" />' +
                                    '<input type="text" class="text" name="newname" value="'+title+'" />' +
                                    '<textarea class="text" name="newdesc">'+description+'</textarea>' +
                                    '</form>';

                    let savebutton = '<a href="#" title="Save changes to category">' +
                    ...
                    // Put it all together
                    tablerow.classList.add("editcat");
                    container.classList.remove('name');
                    container.classList.add('editname');
                    container.innerHTML = editform;
                    this.innerHTML = savebutton;
